...
class PNASNet5LargeEncoder(nn.Module):
    def __init__(self, num_classes=1001):
        super(PNASNet5LargeEncoder, self).__init__()
        self.num_classes = num_classes
        self.conv_0 = nn.Sequential(OrderedDict([
            ('conv', nn.Conv2d(3, 96, kernel_size=3, stride=2, bias=False, padding=1)),
...
def pnasnet5large(num_classes=1001, pretrained='imagenet'):
    ...
    if pretrained:
        ...
        model = PNASNet5LargeEncoder(num_classes=1001)
        ...
        model = PNASNet5LargeEncoder(num_classes=num_classes)
    return model
...
class PNASNet5Large(nn.Module):
    ...
    def forward(self, data: torch.Tensor) -> torch.Tensor:
        ...
        encoder_out = self.encoder.get_features_min(data)

        return encoder_out

if __name__ == '__main__':
    model = PNASNet5Large(ckpt=r'D:\Projects\github\360DN\scripts\train\actions\train\pnas.pth')
    img = torch.rand(5, 3, 256, 512)
    depth = model(img)
    print(depth.shape)